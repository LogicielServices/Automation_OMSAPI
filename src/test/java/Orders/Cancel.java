...
public class Cancel {
										   ...
										   String Status_Validation_Flag)
	{
		try
		{
			...
			Assert.assertEquals(cancel_response.statusCode(),Integer.parseInt(EquityOrder_Cancel_StatusCode), "Order Cancel Response");
			if(Endpoint_Version.equalsIgnoreCase("V1")) {Assert.assertEquals(cancel_response.getBody().asString(), EquityOrder_Cancel_Response,"Verify_EquityOrder_Cancel_Response");}
			else{Assert.assertEquals(cancel_response.jsonPath().get("message"), EquityOrder_Cancel_Response,"Verify_EquityOrder_Cancel_Response");}
